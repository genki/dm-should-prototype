...
- <tt>Item.specs.on(:name)</tt> is an array of SpecClass's.
...
- And <tt>record.errors.on(:name)</tt> is also an array of SpecClass's.

==== Each Spec Class is responsible for validation
- They have <tt>satisfy?</tt> method to ensure themselves

  <tt>satisfy?</tt> method is sended when <tt>record.valid?</tt> .   
  

Finished in 0.019124 seconds
