...
=== DataMapper::Model (Class methods) with dm-should
...
=== DataMapper::Resource (Instance methods) with dm-should
==== valid? method
...
=== Spec Class of dm-should
==== What is SpecClass? For example ..
...
- Item.specs.on(:name) is an array of SpecClass's.
...
- And record.errors.on(:name) is also an array of SpecClass's.
